...
def delete_notes_record(request, id: int):
    try:
        ret = records.models.Record.objects.filter(
            id=id, record_type=records.models.RecordTypes.NOTE, owner_token=request.auth.owner_token).delete()
        if ret[0] == 0:
...
def note_crate_record(request, id: int, new_note: records.schemas.Note = None):
    try:
        if id <= 0:
            raise ValueError('Bad id')
        ret = records.models.Record.objects.filter(
        ...
        ret.update(record_type=records.models.RecordTypes.NOTE)
        if ret == 0:
...
def update_note(request, record_id: int, new_note: records.schemas.Note):
    try:
        if record_id <= 0:
            raise ValueError('Bad id')
        ret = records.models.Record.objects.filter(
            id=record_id, owner_token=request.auth.owner_token, record_type=records.models.RecordTypes.NOTE).update(note=new_note.note)
        if ret == 0:
