...
@router.get('/get_record', response=later.schemas.LaterRecordOut, auth=AuthBearer())
def get_later_record(request, id: int):
    try:
        if id <= 0:
            raise ValueError('Bad id')
        rs = records.models.Record.objects.filter(
            id=id, record_type=records.models.RecordTypes.LATER, owner_token=request.auth.owner_token)
        r = rs[0]
        return r
    except Exception as e:
        print(f'{e}')
...
@router.get('/get_records', response=List[later.schemas.LaterRecordOut], auth=AuthBearer())
def get_later_records(request, limit: int = 100, offset: int = 0):
    if limit > 500:
        limit = 500
    if offset < 0:
        offset = 0
    return list(records.models.Record.objects.filter(record_type=records.models.RecordTypes.LATER, owner_token=request.auth.owner_token).order_by('-id')[offset:(offset+limit)])
...
def delete_later_record(request, id: int):
    try:
        ret = records.models.Record.objects.filter(
            id=id, record_type=records.models.RecordTypes.LATER, owner_token=request.auth.owner_token).delete()
        if ret[0] == 0:
...
def update_note(request, record_id: int, new_note: records.schemas.Note):
    try:
        if record_id <= 0:
            raise ValueError('Bad id')
        ret = records.models.Record.objects.filter(
            id=record_id, record_type=records.models.RecordTypes.LATER, owner_token=request.auth.owner_token).update(note=new_note.note)
        if ret == 0:
